...
5. create two new columns for the id and the month of the year.
...
Remember, that a `track_xy*` behaves like a regular `data.frame`. This means that we can use all data manipulation verbs that we are used to from `base` R or the `tidyverse`. For example, we can filter a track based on some characteristic. As an example we extract all relocations from the month May. 
